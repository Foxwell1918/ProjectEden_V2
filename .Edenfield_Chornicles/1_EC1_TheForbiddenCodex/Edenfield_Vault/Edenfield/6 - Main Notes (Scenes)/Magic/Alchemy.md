---
title: Template Magic
tags:
  - Magic
---
[[The ARCANE]]

*This article is about the fictional system of magic in the  [[Edenfield Chornicles]]

---

## Alchemy

| **Name**                  | Alchemy           |
| ------------------------- | ----------------- |
| **Type**                  | Material-based.   |
| **Source of Power**       | Nature, Arcane.   |
| **Users**                 | Alchemist         |
| **Accessibility**         | Any magicians.    |
| **First Known Use**       | Alamiria          |
| **Notable Practitioners** | [[Casnius Bolos]] |

---

## Overview
Alchemy is a Magic System that alters the state of natural matter.

Alchemical spells change the  properties of a creature , object, an environment. They might turn an enemy into a harmless creature. bolster the strength of an ally, make an object move at the caster's command, ar enhance a creature's innate healing abilities lo rapidly recover from injury.

Alchemy was founded by Casnius Bolos in the Dunes of Alamiria. the land so desolate that it is scientifically unprobably for people to live there, but due to genocide, the people must live there. That is when Casnius Bolos founded how to harness the limited Arcane Energy that flows in the desert, he began to use that little energy to transfer the energy that has already existed in the materials surrounding him. His technique was known and eventually taught throughout the dunes, and eventually be called Alchemy.

---

## Etymology
The word alchemy comes from  Old Argantheian _alkimie_, used in The Dark Age as _alchymia_. This name was itself adopted from the Alamirian word _al-kīmiyā_ (الكيمياء). The Alamirian _al-kīmiyā_ in turn was a borrowing of the late Glyndorian term _khēmeía_ (χημεία), also spelled _khumeia_ (χυμεία) and _khēmía_ (χημία), with Al being the Alamirian definite article 'the',

Together this association can be interpreted as **'the process of Transmutation. by which to fuse or reunite with the divine or original form**'. Several etymologies have been proposed for the Glyndorian term. The first was proposed by Zosimos of Panopolis (3rd–4th centuries), who derived it from the name of a book, the _Khemeu_. Hermann Diels argued in 1914 that it rather derived from χύμα, used to describe metallic objects formed by casting.

---

## Principles and Rules
[Explain the underlying logic of how the magic works — energy sources, limits, laws.]

---

## Methods of Use
### Casting/Activation
[How spells or abilities are performed: gestures, incantations, mental focus, materials.]

### Tools and Components
[Required catalysts, reagents, foci, or technology.]

---

## Types and Sub-Disciplines
[List the main branches, schools, or variations within the system, with descriptions.]

---

## Limitations and Costs
[Physical, mental, or spiritual tolls. What magic *cannot* do. Risks and dangers.]

---

## History
### Origins
[When and how the system was first discovered or invented.]

### Development
[How it evolved over time — changes in knowledge, practice, or technology.]

### Modern Use
[How it’s used in the story’s current era, including societal roles.]

---

## Cultural Significance
[Religious, political, or economic influence of the magic. How society views practitioners.]

---

## Notable Practitioners
- [[Casnius Bolos]] Founder
- **[Name]** – [Brief note on why they are important]

---

## In the Story
[Key moments or plotlines involving this magic system.]

---

## Sanderson’s First Law: "An author’s ability to solve conflict with magic is directly proportional to how well the reader understands said magic."
- **Rules & Mechanics:**  
  - You must know the movement of the molecules and can imagine it, in order to make the change in the object.  
  - By learning each molecules and movements of molecules. Alchemy is like math. think of how some people can quickly answer 12+11 is 23, but some people require to use their fingers or paper. How quick an Alchemist can cast a spell is like how fast a mathematician can come to a conclusion. 
  - Unlike other magical disciplines, what limits an Alchemist is one own's brain power, anyone who can harness magic can learn Alchemy. that is why Alchemy is also called "People's magic."
- **Costs of Use:**  
  - Materials, Brain Power.
- **Strengths of the System:**  
  - It is very empirical and doesn't require rituals, sacrifices, meditations, etc. Someone can know Alchemy whilst not really know much about the Arcane.

---

## Sanderson’s Second Law: "Weaknesses are more interesting than powers."
- **Limitations:**  
  - Alchemy can't transform what the Caster didn't know.
  - Boundaries of the system.  
- **Drawbacks / Side Effects:**  
  - What happens if the system is pushed too far (madness, mutations, burnout, ecological damage).  
- **Vulnerabilities:**  
  - Weaknesses in the practitioner (need training, reliance on tools, corruption, addiction).  
  - Weaknesses in the system itself (counter-magic, null zones, imbalance).  

---

## Sanderson’s Third Law: "Expand what you already have before you add something new."
- **Interconnections:**  
  - How does this magic system connect with other systems (if multiple exist)?  
  - Does it overlap with religion, technology, or politics?  
- **Applications in Daily Life:**  
  - Beyond combat, how is it used in culture, science, trade, or everyday survival?  
- **Creative Extensions:**  
  - Logical extrapolations of what existing rules allow (transport, healing, communication, art).  

---

## In-World Lore
- **Origins:** [How did the system come to be? Divine gift, natural evolution, ancient discovery?]  
- **Cultural Role:** [How societies view magic—revered, feared, regulated, outlawed.]  
- **Myths & Stories:** [Legends tied to the system’s origin and use.]  

---

## Examples of Magic in Action
- **Case Study 1:** [Short scenario showing its use in solving conflict.]  
- **Case Study 2:** [Short scenario showing failure or drawback.]  

---

## References
[Optional citations, in-world or out-of-world.]

---

## See Also
- [[Related Magic System]]
- [[Related Faction or School]]
